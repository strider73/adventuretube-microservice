    ...
    public Mono<Void> filter(ServerWebExchange exchange,
        ...
        if (validator.isSecured.test(request)) {
            ...
            try {
                ...
                log.warn("Expired JWT token for path {}", request.getURI().getRawPath(), e);
                return onError(exchange, HttpStatus.UNAUTHORIZED, "Expired JWT token: gateway-service", "TOKEN_EXPIRED");
            } catch (SignatureException e) {
                log.warn("Invalid JWT signature for path {}", request.getURI().getRawPath(), e);
                return onError(exchange, HttpStatus.UNAUTHORIZED, "Invalid JWT signature: gateway-service",
                        "TOKEN_INVALID_SIGNATURE");
            } catch (MalformedJwtException e) {
                log.warn("Malformed JWT token for path {}", request.getURI().getRawPath(), e);
                return onError(exchange, HttpStatus.UNAUTHORIZED, "Malformed JWT token: gateway-service",
                        "TOKEN_MALFORMED");
            } catch (Exception e) {
                log.error("Unexpected error during JWT validation for path {}", request.getURI().getRawPath(), e);
